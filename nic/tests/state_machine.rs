...
fn test_start_cycle() {
    ...
    let cycle = Cycle { id: 1, instructions: vec![(1, Duration::minutes(30))] };
...
async fn test_scheduler_triggers_modes() {
    ...
    let timeframe = AllowedTimeframe { start: now, end: now + chrono::Duration::hours(1) };
    ...
    *app_state.watering_system.sectors.write().await = sectors;
    ...
    // Initialize Auto Mode with a cycle
    {
        ...
        auto_mode.cycle = Cycle { id: 1, instructions: vec![(1, chrono::Duration::minutes(30))] };
    }
    ...
    let task = tokio::spawn({
        ...
        async move {
            ...
            for _ in 0..5 {
                ...
                if let Some(next_start) = wizard_mode.calculate_next_start(time, timeframe) {
                    if time >= next_start {
                        wizard_mode.execute(&app_state.watering_system, now.date_naive(), &app_state.db).await;
                    }
                }
            }
        }
    ...
    assert!(app_state.watering_system.sectors.read().await.get(&1).unwrap().progress > 0.);
}
