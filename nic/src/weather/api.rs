...
pub async fn list_devices<C:SensorController>(State(_app_state): State<Arc<AppState<C>>>) -> Json<Vec<String>> {
    // Fetch devices from DB or MQTT
    Json(vec!["Device1".to_string(), "Device2".to_string()])
}

pub async fn query_weather<C: SensorController>(State(_app_state): State<Arc<AppState<C>>>) -> Json<serde_json::Value> {
