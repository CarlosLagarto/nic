...
pub async fn monitor_udp<D: DatabaseTrait + 'static>(
    ...
    loop {
        ...
        if let Ok(data) = serde_json::from_slice::<serde_json::Value>(&buf[..len]) {
            ...
            tx.send(ControlSignal::Weather(data.to_string())).unwrap();
        }
    }
}

#[allow(clippy::single_match)]
pub async fn monitor_mqtt(tx: Arc<broadcast::Sender<ControlSignal>>) {
        ...
        match event {
            Event::Incoming(Packet::Publish(publish)) => {
                if let Ok(msg) = String::from_utf8(publish.payload.to_vec()) {
                    tx.send(ControlSignal::DevicesState(msg)).unwrap();
                }
            }
