...
impl ModeEnum {
    pub async fn execute<C: SensorController + 'static, D: DatabaseTrait + 'static>(
        &mut self, water_sys: &mut WateringSystem<C>, db: &Arc<D>, current_time: DateTime<chrono::Utc>,
    ) {
        match self {
            ...
            ModeEnum::Wizard(mode) => {
                let current_date = current_time.date_naive(); // Extract current date
                mode.execute(water_sys, current_date, db).await;
            }
        }
    }
}
